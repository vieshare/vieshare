...
#define VIESHARE_LIB_PATH "libvieshare.so"
typedef bool (*VieShareCoreMain)();
bool gIsConnectionManager = false;

void print_help_install_pkg(const char* so);

bool flutter_vieshare_core_main() {
   void* libvieshare = dlopen(VIESHARE_LIB_PATH, RTLD_LAZY);
   if (!libvieshare) {
      fprintf(stderr,"Failed to load \"libvieshare.so\"\n");
      char* error;
   ...
   auto core_main = (VieShareCoreMain) dlsym(libvieshare,"vieshare_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
       fprintf(stderr, "Program entry \"vieshare_core_main\" is not found: %s\n", error);
       return false;
   }
   return core_main();
}
