...
#
# Script to build F-Droid release of VieShare
#
# Copyright (C) 2024, The VieShare Authors
...
arm64-v8a)
	...
	VIESHARE_FEATURES='flutter,hwcodec'
	;;
armeabi-v7a)
...
prebuild)
	...
	# Flutter used to compile main Vieshare library
	...
	# Install Flutter version for VieShare library build
...
build)
	...
	cargo ndk \
		...
		build \
		--release \
		--features "${VIESHARE_FEATURES}"

	mkdir -p "flutter/android/app/src/main/jniLibs/${ANDROID_ABI}"

	cp "target/${RUST_TARGET}/release/liblibvieshare.so" \
		"flutter/android/app/src/main/jniLibs/${ANDROID_ABI}/libvieshare.so"
