...
class PlatformFFI {
  ...
  VieshareImpl get ffiBind => _ffiBind;
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = isAndroid
        ? DynamicLibrary.open('libvieshare.so')
        : isLinux
            ? DynamicLibrary.open('libvieshare.so')
            : isWindows
                ? DynamicLibrary.open('libvieshare.dll')
                :
                ...
                //
                // isMacOS? DynamicLibrary.open("liblibvieshare.dylib") :
  ...
  void _startListenEvent(VieshareImpl vieshareImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = vieshareImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
