...
List<TTextMenu> toolbarControls(BuildContext context, String id, FFI ffi) {
  ...
  // game controls
  if (isMobile && isDefaultConn && !ffiModel.viewOnly && ffiModel.keyboard) {
    v.add(TTextMenu(
      child: Text(translate('Game')),
      onPressed: () => showGameControlsDialog(ffi),
    ));
  }
  // fingerprint
...
bool showVirtualDisplayMenu(FFI ffi) {
  ...
  if (ffi.ffiModel.pi.isVieShareIdd || ffi.ffiModel.pi.isAmyuniIdd) {
    return true;
  }
  return false;
}
...
List<Widget> getVirtualDisplayMenuChildren(
  ...
  if (pi.isVieShareIdd) {
    final virtualDisplays = ffi.ffiModel.pi.VieShareVirtualDisplays;
...
void showGameControlsDialog(FFI ffi) {
  ffi.dialogManager.show((setState, close, context) {
    return CustomAlertDialog(
      title: Text(translate('Game Controls')),
      content: Obx(() {
        final isLoLActive = ffi.gameControlsModel.isLoLMode;
        return Column(
          ...
          children: [
            ListTile(
              leading: Icon(
                ...
                color: isLoLActive ? Colors.green : MyTheme.accent
              ),
              title: Text('League of Legends'),
              subtitle: Text(isLoLActive 
                ? 'Essential controls active: Joystick + Q/W/E/R/D/F/B + Screen lock' 
                : 'Enable streamlined LoL controls for mobile gameplay'),
              trailing: isLoLActive ? Icon(Icons.check_circle, color: Colors.green) : null,
              onTap: () {
                close();
                if (isLoLActive) {
                  ffi.gameControlsModel.disableGameMode();
                } else {
                  ffi.gameControlsModel.enableLoLMode();
                }
              },
            ),
            if (isLoLActive) Divider(),
            if (isLoLActive) ListTile(
              leading: Icon(Icons.settings, color: MyTheme.accent),
              title: Text('LoL Settings'),
              subtitle: Text('Auto-detection settings'),
              onTap: () {
                close();
                _showSimpleLoLSettings(context, ffi);
              },
            ),
            if (isLoLActive) ListTile(
              leading: Icon(Icons.close, color: Colors.red),
              title: Text('Disable Game Controls'),
              subtitle: Text('Return to normal controls'),
              onTap: () {
                close();
                ffi.gameControlsModel.disableGameMode();
              },
...
void _showSimpleLoLSettings(BuildContext context, FFI ffi) {
  final gameModel = ffi.gameControlsModel;
  
  gFFI.dialogManager.show((setState, close, context) {
    return CustomAlertDialog(
      title: Row(
        children: [
          Icon(Icons.sports_esports, color: MyTheme.accent),
          SizedBox(width: 8),
          Text('League of Legends Settings'),
        ],
      ),
      content: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          Text(
            'Essential controls: Movement joystick, Q/W/E/R skills, D/F summoner spells, B for shop, and screen lock.',
          ...
          Obx(() => SwitchListTile(
            ...
            subtitle: Text('Automatically enable when League of Legends is detected'),
            value: gameModel.autoDetectionEnabled,
            onChanged: (value) => gameModel.setAutoDetection(value),
            dense: true,
          )),
        ],
      ),
