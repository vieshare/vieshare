...
typedef char** (*FUNC_VIESHARE_CORE_MAIN)(int*);
typedef void (*FUNC_VIESHARE_FREE_ARGS)( char**, int);
typedef int (*FUNC_VIESHARE_GET_APP_NAME)(wchar_t*, int);
...
{
  HINSTANCE hInstance = LoadLibraryA("libvieshare.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libvieshare.dll." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_VIESHARE_CORE_MAIN vieshare_core_main =
      (FUNC_VIESHARE_CORE_MAIN)GetProcAddress(hInstance, "vieshare_core_main_args");
  if (!vieshare_core_main)
  {
    std::cout << "Failed to get vieshare_core_main." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_VIESHARE_FREE_ARGS free_c_args =
      (FUNC_VIESHARE_FREE_ARGS)GetProcAddress(hInstance, "free_c_args");
  if (!free_c_args)
  {
  ...
  int args_len = 0;
  char** c_args = vieshare_core_main(&args_len);
  if (!c_args)
  {
    std::string args_str = "";
    ...
    // std::cout << "VieShare [" << args_str << "], core returns false, exiting without launching Flutter app." << std::endl;
    return EXIT_SUCCESS;
  }
  std::vector<std::string> rust_args(c_args, c_args + args_len);
  free_c_args(c_args, args_len);

  std::wstring app_name = L"VieShare";
  FUNC_VIESHARE_GET_APP_NAME get_vieshare_app_name = (FUNC_VIESHARE_GET_APP_NAME)GetProcAddress(hInstance, "get_vieshare_app_name");
  if (get_vieshare_app_name) {
    wchar_t app_name_buffer[512] = {0};
    if (get_vieshare_app_name(app_name_buffer, 512) == 0) {
