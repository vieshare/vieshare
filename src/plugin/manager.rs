...
fn get_plugin_source_list() -> Vec<PluginSource> {
    ...
    // vec![PluginSource {
    //     name: "vieshare".to_string(),
    //     url: "https://raw.githubusercontent.com/fufesou/vieshare-plugins/main".to_string(),
...
fn elevate_install(
    ...
    crate::platform::elevate(args, "VieShare wants to install then plugin")
}
...
fn elevate_uninstall(plugin_id: &str) -> ResultType<bool> {
    crate::platform::elevate(
        ...
        "VieShare wants to uninstall the plugin",
    )
}
