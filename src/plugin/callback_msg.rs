...
const MSG_TO_VIESHARE_TARGET: &str = "vieshare";
...
const MSG_TO_VIESHARE_SIGNATURE_VERIFICATION: &str = "signature_verification";
...
pub(super) extern "C" fn cb_msg(
    ...
    match &target as _ {
        ...
        MSG_TO_VIESHARE_TARGET => handle_msg_to_vieshare(id, content, len),
        _ => PluginReturn::new(
...
fn handle_msg_to_vieshare(id: String, content: *const c_void, len: usize) -> PluginReturn {
    let s = early_return_value!(
        std::str::from_utf8(unsafe { std::slice::from_raw_parts(content as _, len) }),
        ERR_CALLBACK_INVALID_MSG,
        "parse msg string"
    );
    let msg_to_vieshare = early_return_value!(
        serde_json::from_str::<MsgToVieShare>(s),
        ERR_CALLBACK_INVALID_MSG,
        "parse msg '{}'",
        s
    );
    match &msg_to_vieshare.r#type as &str {
        MSG_TO_VIESHARE_SIGNATURE_VERIFICATION => request_plugin_sign(id, msg_to_vieshare),
        t => PluginReturn::new(
            errno::ERR_CALLBACK_TARGET_TYPE,
            &format!(
                "Unknown target type '{}' for target {}",
                t, MSG_TO_VIESHARE_TARGET
            ),
        ),
    }
}

fn request_plugin_sign(id: String, msg_to_vieshare: MsgToVieShare) -> PluginReturn {
    let signature_data = early_return_value!(
        std::str::from_utf8(&msg_to_vieshare.data),
        ERR_CALLBACK_INVALID_MSG,
