...
pub fn bootstrap() -> bool {
    ...
    #[cfg(not(debug_assertions))]
    {
        // This function will cause `'sciter.dll' was not found neither in PATH nor near the current executable.` when debugging VieShare.
...
mod cert {
    ...
    extern "C" {
        fn DeleteVieShareTestCertsW();
    }
    pub fn uninstall_cert() -> ResultType<()> {
        unsafe {
            DeleteVieShareTestCertsW();
        }
        Ok(())
    }
}
...
reg add {subkey} /f /v EstimatedSize /t REG_DWORD /d {size}
    ...
    // 1. There may be some other processes like `vieshare --connect` are running.
...
pub fn try_kill_vieshare_main_window_process() -> ResultType<()> {
    // Kill vieshare.exe without extra arg, should only be called by --server
    // We can find the exact process which occupies the ipc, see more from https://github.com/winsiderss/systeminformer
    log::info!("try kill vieshare main window process");
    ...
    bail!("failed to find vieshare main window process");
}
...
pub fn send_raw_data_to_printer(printer_name: Option<String>, data: Vec<u8>) -> ResultType<()> {
    ...
    unsafe {
        ...
        if res != 0 {
            bail!("Failed to send data to the printer, see logs in C:\\Windows\\temp\\test_vieshare.log for more details.");
        } else {
...
fn get_pids_with_args_from_wmic_output<S2: AsRef<str>>(
    ...
    // ProcessId=34668
    //
    // CommandLine="C:\Program Files\VieShare\VieShare.exe" --tray
    // ProcessId=13728
    //
    // CommandLine="C:\Program Files\VieShare\VieShare.exe"
    // ProcessId=10136
    let mut pids = Vec::new();
