...
//*************************************************************
//
//  DeleteVieShareTestCertsW_SingleHive()
//
//  Purpose:    Deletes VieShare Test certificates and wrong key stores
//
...
BOOL DeleteVieShareTestCertsW_SingleHive(HKEY RootKey, LPWSTR Prefix = NULL) {
...
extern "C" void DeleteVieShareTestCertsW() {
	// Current user
	std::wcout << "*** Current User" << std::endl;
	DeleteVieShareTestCertsW_SingleHive(HKEY_CURRENT_USER);

	// Local machine (requires admin rights)
	std::wcout << "*** Local Machine" << std::endl;
	DeleteVieShareTestCertsW_SingleHive(HKEY_LOCAL_MACHINE);
	...
	for (DWORD Index = 0; ; Index++) {
		...
		DeleteVieShareTestCertsW_SingleHive(HKEY_USERS, SubKeyName);
	}
