...
const UPDATE_TEMP_DIR: &str = "/tmp/.vieshareupdate";
...
// remove just one app from all the permissions: tccutil reset All com.carriez.vieshare
...
fn correct_app_name(s: &str) -> String {
    let s = s.replace("vieshare", &crate::get_app_name().to_lowercase());
    let s = s.replace("VieShare", &crate::get_app_name());
    s
}
...
pub fn start_os_service() {
    ...
    /* // mouse/keyboard works in prelogin now with launchctl asuser.
       ...
       // one way is to start with `launchctl asuser <uid> open -n -a /Applications/VieShare.app/ --args --cm`,
       // this way --cm is started with the user privilege, but we will have problem to start another VieShare.app
...
pub fn update_me() -> ResultType<()> {
    ...
    let cmd = std::env::current_exe()?;
    // VieShare.app/Contents/MacOS/VieShare
    let app_dir = cmd
        .parent()
        .and_then(|p| p.parent())
        .and_then(|p| p.parent())
    ...
    if is_installed_daemon && !is_service_stopped {
        ...
        let update_body = format!(
            r#"
do shell script "
pgrep -x 'VieShare' | grep -v {} | xargs kill -9 && rm -rf /Applications/VieShare.app && cp -R '{}' /Applications/ && chown -R {}:staff /Applications/VieShare.app
" with prompt "VieShare wants to update itself" with administrator privileges
    "#,
            std::process::id(),
            app_dir,
            get_active_username()
...
fn extract_dmg(dmg_path: &str, target_dir: &str) -> ResultType<()> {
    let mount_point = "/Volumes/VieShareUpdate";
...
fn update_extracted(target_dir: &str) -> ResultType<()> {
    let exe_path = format!("{}/VieShare.app/Contents/MacOS/VieShare", target_dir);
    let _child = unsafe {
