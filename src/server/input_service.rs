...
impl VirtualInputState {
    fn new() -> Option<Self> {
        VirtualInput::new(
            ...
            // 1. Press "Command" key in VieShare
            // 2. Move the physical mouse
            // 3. Press "V" key in VieShare
            ...
            // we consider only VieShare operates the controlled side.
            //
...
#[cfg(target_os = "macos")]
fn key_sleep() {
    // https://www.reddit.com/r/vieshare/comments/1kn1w5x/typing_lags_when_connecting_to_macos_clients/
    //
    ...
    // Though `/Applications/VieShare.app/Contents/MacOS/vieshare --server` in terminal is ok.
    let now = Instant::now();
...
pub async fn lock_screen() {
    ...
    if #[cfg(target_os = "linux")] {
        ...
        // loginctl lock-session also not work, they both work run vieshare from cmd
        std::thread::spawn(|| {
